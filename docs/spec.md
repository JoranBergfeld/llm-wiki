...
- No PDF/image/URL ingestion (markdown and plain text only). **Narrowed by amendment Y:** this rules out *ingestion* — nothing is ever fetched, archived, or stored from a URL. `wiki links check --external` probes liveness only, and is opt-in.
...
lint:
  ...
  content_loss_percent: 25       # optional (default 25): a full-body update dropping this share of a page's references files a content-loss issue
```
...
- Exit codes: `0` success · `1` blocking validation failure (input rejected, nothing written) · `2` environment/IO error · `3` state conflict (e.g., resuming a ledger step already done — safe, idempotent no-op reported) · `4` a measurement came in under an explicitly requested threshold (`wiki eval --fail-under` only, amendment W — never produced by any other command).
...
wiki category list
wiki category propose <id> --description "…"    Agent-proposable (amendment U): records a proposal citing the
                [--rationale "…"] [--sources …]  source ids that fit no existing category. Never writes wiki.yaml.
wiki category proposals [--status …]
wiki category approve <proposal-id>             Human applies it; performs the equivalent `category add`
wiki category reject <proposal-id> [--note "…"]
...
wiki source scan <dir> --category <id> [--dry-run]
                                                Bulk registration of an inbox directory (amendment T): registers
                                                every not-yet-registered file, recursively, one category per inbox.
                                                Idempotent (content hash dedup); per-file outcomes reported in the
                                                envelope; one rejected file never aborts the batch.
...
wiki links check [--external] [--timeout <ms>]   External URL liveness (amendment Y). Offline INVENTORY by
                 [--concurrency <n>]              default; --external opts into the network. Files
                                                 broken-external-link issues for definitive failures only.
                                                 Never part of lint, never a precondition, never blocks a write.
wiki audit next                                 Faithfulness auditing (amendment X): CLI deterministically
wiki audit record <page-id> --verdict …          selects a page + its cited source ids; the AGENT judges,
                --note "…"                       cold and adversarially; the CLI records. `unsupported`
wiki audit list [--verdict …]                    files an issue. No model in the binary. Never in lint.
wiki eval [--k N] [--fail-under N]              Golden-question retrieval scoring (recall@k) against the
                                                human-owned eval.yaml (amendment W). NEVER part of lint.
                                                Exits 0 when reporting; exits 4 only under --fail-under.
...
| `linted` | Post-ingest lint ran clean or issues filed | A lint run newer than the `integrated` timestamp exists. The last-lint timestamp is stored in `.wiki/lint.json` (written by every `wiki lint`); the ledger records the `integrated` timestamp for comparison. Where the ledger holds **no** `integrated` timestamp — a reindex-derived entry, which never witnessed the transition (amendment A) — any recorded lint run satisfies it (amendment Z). |
...
- **T. `wiki source scan`: bulk registration, one category per inbox.** (issue #8) Registration was manual and one-at-a-time, which conflated **editorial judgement** ("does this belong in my knowledge base" — genuinely the human's) with **mechanical registration** (typing the command — pure friction, with no gate value, because sha256 dedup, the ledger, `source impact` and `source retract` already make a bad registration cheap to detect and cheap to undo). `wiki source scan <dir> --category <id>` moves the human *upstream*: they drop files into an inbox and curation becomes "what you put in the folder" rather than "what you type". It is **idempotent by construction** — content is hash-deduped, so re-scanning is a no-op, which is what makes it safe on cron/Task Scheduler/every agent tick and removes any need for a filesystem watcher (watchers are flaky across OSes and effectively untestable). **The category/title question is settled conservatively:** `--category` is required and titles are derived deterministically from filenames. Registering under a placeholder "unresolved" category would need a reserved id the human's closed taxonomy does not contain — breaking `HasCategory` and amendment N's category-in-use rule, and putting a CLI-owned magic value into a human-owned config — and it would not actually reuse the review gate, which gates *pages*, not source registrations. So the **inbox directory is the editorial decision**: one inbox per category. A provisional title costs nothing, because the agent rewrites it into the summary page's title on the very next ingest step. Other settled details: the scan is **recursive** (real inboxes acquire subfolders on their own; `--dry-run` plus dedup keeps that safe), dot-prefixed files and directories are skipped as tooling artefacts, empty/whitespace-only files are skipped rather than registered, `origin` records the file's path within the inbox instead of the `"manual"` default, an inbox **inside the vault** is rejected (`scan-dir-in-vault`) because it would launder the vault's own generated output back in as source material, and **failure isolation** is absolute: a rejected file is reported as its own entry carrying the error code `source add` would have exited with, and the command still exits 0 — the batch ran, and what happened to each file is data, not a failure of the scan. The `AGENTS.md` template and the shipped skill are amended in the same change: the agent may now perform registration from a directory the human has *named as an inbox*, and may still never decide what belongs in the vault.
- **U. `wiki category propose`: categories get the propose/approve channel every other human-gated thing already had.** (issue #9) Categories remain the human's — if the agent could mint them directly the taxonomy would drift into `article` / `articles` / `blog-post` / `blogpost` within a week, which is precisely the decay this project exists to prevent, and nothing here changes that. What is closed is an **asymmetry**: pages route through the review gate (§15) and `AGENTS.md` routes through the reflect loop (§13), but categories had neither — the agent hit `unknown-category` and the only available instruction was "report it", leaving the human to work out unaided what the new category should be and why. `wiki category propose <id> --description "…" [--rationale "…"] [--sources id1,id2]` records a proposal; `proposals`/`approve`/`reject` mirror `schema`'s verbs exactly; approving performs the `category add` the human would otherwise have typed, and is still the only route to a `wiki.yaml` write. **The `--sources` list is the point:** citing the source ids that fit no existing category is what makes the decision reviewable rather than a judgement call about a name in the abstract, the same role recurring issue ids play in a `schema propose` rationale, so those ids are validated against the idmap (`unknown-source`) rather than accepted as prose. **Has `category` earned its gate?** The issue asks that first, and the answer moved while these issues were being worked: before amendment T, `category` was consumed only by source frontmatter, `source list --category` and amendment N's in-use check — thin enough that open-set `tags` might have been the better primitive. `wiki source scan` makes a category the **routing key for an entire inbox directory**, an editorial decision the human makes once instead of per file. That is a real job, so the gate stays. **Separate store, not a shared one.** `.wiki/category-proposals.json` is its own file with its own type. The shapes are not close enough to share: a schema proposal is (section heading, new full section text) applied with `SectionLocator.Replace` against `AGENTS.md`; a category proposal is (kebab id, description, cited sources) applied with `CategoryService.Add` against `wiki.yaml`. Sharing would mean overloading `Section` as a category id and `NewText` as a description, and it would then leak — `wiki schema proposals` would list category proposals as `AGENTS.md` amendments, and `wiki schema approve` on one would fail trying to locate a section named after a category. Same pattern, separate state. Separately and needing no code: *suggesting an existing* category at registration time is `wiki category list --json` plus the source content, so the ingest playbook now says to do that first and to propose only when nothing fits.
- **V. `content-loss`: generation loss gets a detectable signature, on the write path only.** (issue #11, part C) Every §11 lint check is a pure function of the vault's *current* bytes, which is exactly why none of them can see a fact that was there last week and is not there now: the replaced body is gone by the time lint runs. Only `page upsert --id` holds both versions, so only the upsert can measure the loss. That matters because `--id` replaces the entire body, so every re-integration is a full rewrite by a model holding the old body plus one new source — a fact from the first source can evaporate on the pass that integrates the seventh, and in a retrieval-first vault that surfaces as a **false negative**: the right page is returned and the fact you needed is silently gone. "The vault never knew that" and "the vault forgot it on Tuesday" are indistinguishable at the point of use. **How removal is measured, settled:** structurally — which wikilink targets and which cited source ids the page carried before and no longer carries — not by line diff. A naive line diff scores every legitimate rewrite as near-total deletion, which makes the signal useless; counting references maps directly onto "this page used to connect to X and no longer does", is cheap, and is exactly the dumb structural proxy the existing checks already trade in (amendment F). It explicitly does **not** claim to catch a fact stated in prose with no link attached — nothing deterministic can — but it catches the version of that failure that leaves a footprint, which is most of the ones that matter in a graph the agent navigates by link. Old/new body line counts ride along in the envelope as unthresholded context. **The report is in the envelope on every update**, so the agent sees it in the same tick that caused it and can self-correct immediately; that feedback loop is most of the value, and an issue filed for a human to read days later is worth much less. A `content-loss` issue (its own `IssueKind`, per amendment H's collision rule) is filed only above `lint.content_loss_percent` — a new **optional** `wiki.yaml` key defaulting to 25, optional because a config that was valid yesterday must not brick the CLI today. Legitimate large deletions exist (splitting an `oversize` page), so the finding never blocks the write and is resolvable with a note like any other issue kind.
- **W. `wiki eval`: a deterministic retrieval-quality signal, and a new exit code for it.** (issue #11, part A) Every lint check is deliberately non-semantic and says so in its own comments, which means **a vault of well-linked, correctly-sized lorem ipsum passes the full suite**. That is the right design — determinism is why the CLI/LLM split works — but it left no quality signal at all, and volume stops being capped by human attention the moment an inbox is scanned on a schedule (amendment T): failures tolerable across 20 hand-curated sources are invisible across 500 auto-ingested ones, and the failure mode is silent and monotone. `wiki eval` scores the vault's routing against golden questions in a human-owned `eval.yaml` at the vault root. **The metric falls out of the existing design:** the retrieval playbook budgets "at most 10 candidate pages", so the measure is **recall@k with k defaulting to 10**. Expectations are "must include", never exact sets, so they survive vault growth instead of breaking every time a page is added. **Routing is simulated deliberately dumbly** (amendment F): `score = 3 × (distinct question terms in title+summary) + 1 × (distinct terms in body)`, zero-scoring pages are not candidates, ties break on slug ordinal, archived pages are excluded as `index.md` excludes them. The 3:1 weighting is the point rather than a tuning knob — `summary` is the most important text in the vault because it is what `index.md` routes on and the playbook forbids scanning bodies to find relevance, so a vague summary does not degrade a page, it makes the page **invisible**. That is a retrieval bug, not a prose-style issue, and weighting the routing surface above the body is what makes it show up in the score. **Ownership:** the file is human-owned and the CLI never writes it — the "CLI is the only writer to the vault" invariant leaves the agent no route to author it anyway, and letting the model write its own exam defeats the point. Agent-proposed questions, if ever wanted, route through a propose/approve channel like amendment U's. **Exit code, decided explicitly:** a failing eval is not "your input was rejected", so reusing exit 1 would muddy the one contract the agent branches on. Reporting is always exit 0; the opt-in `--fail-under <score>` exits **4**, a code no other command produces, meaning only "the measurement came in under the bar you set" — and the full report is still emitted, because a caller that set a bar needs to know *which* questions missed. **None of this goes into `wiki lint`.** Lint's value is that it is deterministic, free, and that "clean" means something precise; mixing in a measurement that depends on a human-authored question file would spend that asset. `eval` is its own command, and content-loss is filed like any other issue kind.
- **X. `wiki audit`: the agent judges, the CLI records.** (issue #12) Amendments V and W cover *retrieval* (does the router surface the right pages) and *removal* (what a rewrite dropped, structurally). Neither can say whether a page's claims are actually supported by its cited sources, and that is the failure that matters most in a retrieval-first vault: a confidently wrong page gets cited later without rechecking, which is the entire reason for building the wiki. It also covers **citation rot** (`--sources` is a list of ULIDs and nothing verified the claims derive from them) and **empty calories** (a page that restates its wikilinks without carrying a thesis — well-linked, right-sized, fresh, and saying nothing). All three need semantic judgement. **The CLI having no LLM access is the shape of the feature, not a blocker**, and it is the same split every other part of the system already uses: the CLI does selection and bookkeeping (`wiki audit next` → `wiki audit record`), the agent is the model, the human disposes. No model in the binary, no network call, nothing nondeterministic behind the wall. Findings land as issues, so a recurring semantic finding is exactly the evidence the reflect loop was built to act on. **Retrieval-first makes it cheap:** faithfulness only needs to hold for pages that actually get retrieved, so this is never a whole-vault sweep — which makes **selection** the interesting design problem, not judgement. The best signal would be real retrieval frequency, which needs a retrieval log that does not exist; that log is deliberately **not** built here — it would also let amendment W's golden questions be harvested from real usage instead of hand-authored, so it has to earn its keep on its own merits rather than as a dependency of this. Selection therefore uses heuristics the vault already has, in priority order: pages carrying an open `content-loss` issue (spending the expensive check where the free structural one already pointed — the reason C shipped before this), then never-audited/least-recently-audited, then most cited sources, then slug ordinal so two runs on an unchanged vault pick the same page. Pages citing **no** sources are not candidates at all: there is nothing to check claims against. **`--note` is required for an `unsupported` verdict** — "this page asserts X, no cited source says X" is the entire content of the report, and a finding nobody can act on is worse than none. **False positives are expected.** A verdict is a finding for a human to weigh, not a fact: it gates nothing, and `unsupported-claim` is resolvable with a note like every other kind. The adversarial framing — receive the page and its sources cold, with none of the authoring context, and be prompted to refute rather than confirm — lives in the agent instructions, because it is an instruction to the model and the CLI cannot enforce it. Sampling, not sweeping: one page per tick, consistent with the one-unit-of-work-per-tick rule. `.wiki/audits.json` holds the last verdict per page id (not slug — a rename keeps the id, and an audit is a statement about the page, not its filename); it is historical state in amendment A's sense, so `reindex` leaves it alone.
- **Y. `wiki links check`: external URL liveness as an opt-in command, never as a lint check.** (issue #2) `[[wikilinks]]` are already checked deterministically at write time; external URLs got none of that — a page could cite a URL that 404s and nothing noticed, so provenance for external material was asserted and never verified. **Why it must not go into `wiki lint`:** every existing check is a pure function of the vault's bytes (same vault in, same findings out, offline, instantly). An HTTP probe breaks all four — network, slow, and non-deterministic (429s, redirects, captive portals, sites that 403 anything without a browser UA). The real damage would be to the reflect loop: issues merge on `(kind, subject)` and `occurrences` is the signal that a deficiency is *recurring* (§12), so a finding a flaky network can produce turns `occurrences` from "this problem persists" into "the wifi flaked again", corrupting the one measurement the amendment workflow reads. It also cannot gate the ledger — the `linted` precondition compares `.wiki/lint.json` against a source's `integrated` timestamp, and making ingest completion depend on reachability would strand sources for reasons unrelated to the vault. So: separate command, opt-in, never a precondition, never touching `lint.json` or the ledger, never blocking a write. **Settled open questions.** *(1) New `IssueKind`.* `broken-external-link`, distinct from `dangling-link` — reusing that kind would collide on the `(kind, subject)` merge key and corrupt the wikilink record's detail/occurrences, exactly the failure amendment H documents. There is a test for a page carrying both at once. *(2) Result caching: not implemented, deliberately.* A TTL cache in `.wiki/` is derived state `reindex` would need a story for, and its correctness would hinge on wall-clock — precisely the nondeterminism this command exists to keep out of everything else. The rudeness it would mitigate is handled the cheap way instead: probing is per **distinct URL**, so a link repeated across the vault costs one request, and `--concurrency` (default 4) bounds the burst. *(3) What counts as broken.* Only 404, 410, and DNS/connection failure. 403, 429, timeouts and 5xx are `unverified` — reported but **never filed**, because a soft state is more honest than a false positive and only definitive failures earn a place in the queue an agent works. *(4) Does this reach the AOT binary?* Yes, and the network is still opt-in *within* the command: without `--external` it is a pure offline inventory that opens no socket, which also means the whole feature is usable and testable with no network at all. `System.Net.Http` is AOT-supported and TLS comes from the platform (SChannel/OpenSSL) rather than being statically linked, so no certificate stack is pulled in. **The binary-size delta was measured on CI's four-RID publish and is real: roughly +1.5–1.8 MB compressed, about +50%, on every target** (win-x64 3.36→4.90 MB, linux-x64 3.60→5.43 MB, linux-arm64 3.44→5.21 MB, osx-arm64 3.54→5.27 MB; same packaging both runs, and the links-check change was the only code between them). That is substantially more than the "modest" cost this was expected to carry, and it is paid by every user for one opt-in diagnostic. **Accepted anyway, deliberately:** ~5 MB is still small in absolute terms for a single-file native binary with no runtime to install, which is the property §16 actually cares about, and the alternative — conditional compilation behind an MSBuild switch — would mean the shipped default binary and a locally-built one disagree about which commands exist, which is a worse contract for an agent to reason about than a larger download. Revisit if a second network-shaped feature ever wants in, or if the absolute size starts mattering for distribution. *(5) Spec placement.* §2's "no PDF/image/URL ingestion" non-goal is narrowed above rather than contradicted: it rules out *ingestion*, and nothing here fetches, archives, or stores anything behind a URL. This is liveness only.

---

## Appendix D — Recovery amendments (2026-09-05)

Same standing as Appendices B and C: these override the original text where they conflict. (Single letters end at Z; a further amendment continues `AA`.)

- **Z. A null `integratedAt` is an unwitnessed integration, and any recorded lint run clears the `linted` precondition.** (issue #23) Two individually-correct behaviours collided into a permanently unadvanceable ledger entry. `Ledger.Reconcile` promotes a reindex-derived entry to `integrated` on the entity/concept citations in the markdown — which genuinely do prove integration — but leaves `IntegratedAt` null rather than stamp a transition timestamp it never observed, because fabricating one is exactly what amendment A forbids. `CheckLintPrecondition` then read that null as "no lint ran at or after integration" and rejected. **Those are not the same statement.** The precondition exists to assert an ORDERING (this lint ran after this integration); a null means the other side of the comparison does not exist, so no lint run — past, present, or future — could ever satisfy it. The entry was stuck at `integrated` forever, `advance` was precondition-gated, `source retract` touches source status rather than ledger state, and `reindex` merge-preserves the null rather than reseeding it, so **nothing in the CLI could repair it**. That is sharpest because `reindex` is documented as *the* recovery tool: the repair path created a state no command could repair, and the error told the operator to "run `wiki lint`", naming a remedy that cannot work however many times it is tried. **The fix is to drop the ordering claim, and only where there is nothing to order against.** A null `IntegratedAt` on an entry at `integrated` or beyond is an *unwitnessed* integration; a recorded lint run of any age satisfies the precondition. The substance survives, because "a lint actually ran" is still checked unconditionally — `linted` remains unreachable in a vault that has never been linted, which after a `.wiki/` wipe is the state the vault is in until `wiki lint` runs, so the canonical recovery is still reindex-then-lint-then-advance. Accepting the null is also the reading *consistent* with amendment A's honesty argument rather than against it: refusing to claim an ordering you cannot know is the same instinct as refusing to invent the timestamp in the first place. **Rejected alternatives.** A distinct error code (`integration-unwitnessed`) would let an agent branch instead of looping, but branch to *what* — the entry stays unadvanceable, the work queue stays non-empty, and `ingest status` keeps reporting outstanding work that can never be completed. A `wiki ingest repair` command or `advance --force` would hand the agent a general escape hatch around every precondition to solve one specific accounting gap, which trades a wedged row for a blunt instrument that undermines the gating design everywhere else. **Two adjacent cases are deliberately NOT folded in.** A non-null timestamp that will not parse is corrupt state rather than absent history, and stays rejected — with its own message, because advising a lint re-run there is the same misdirection in a different costume. And a source not integrated *yet* also has a null `IntegratedAt`, but its transition is still ahead of it and will stamp one, so `entry.State >= Integrated` is what distinguishes the two; `ingest resume`'s `expectedArtifacts` respects that split, since the resume plan is the instruction sheet an agent reads after losing context and naming an unsatisfiable goal there is how the loop started. **Why the cost is worth stating:** anything treating `ingest status` as a work queue never goes idle with a wedged row in it. The reporting deployment had an hourly sweep waking an LLM agent whenever the queue was non-empty, so one unadvanceable entry bought an agent session every hour, indefinitely, at real cost — a bug whose blast radius is measured in dollars per day rather than in incorrect output.
