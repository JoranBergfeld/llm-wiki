...
public class LifecycleTests
{
    ...
    public void FullLifecycle_InitThroughRetract_EveryStepExitsCleanAndLandsExpectedState()
    {
        ...
        // (spec §10, amendment D) requires .wiki/lint.json's `lastRun`
        // STRICTLY newer than this entry's `integratedAt` - both are
        // second-precision ISO timestamps off the real wall clock (neither
        // IngestService nor LintService/LintCommand has a clock-injection
        // seam wired through the CLI), so a lint run landing in the SAME
        // wall-clock second as the integrated advance above would make this
        // precondition intermittently fail. That's a real, pre-existing
        // timing edge (not something to special-case in production code for
        // one test) - the brief-sanctioned fix is on the test side: sleep
        // past the second boundary before running lint.
        Thread.Sleep(1100);
