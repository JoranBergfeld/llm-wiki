...
public static class IssuesCommand
{
    ...
    private static Command BuildList(Option<string?> vaultOption, Option<bool> jsonOption, TextWriter stdout, TextReader stdin)
    {
        ...
        {
            Description = "Filter to a single issue kind (orphan | dangling-link | stale | coverage-gap | index-drift | oversize | rename-drift | needs-review-backlog | pending-backlog | review-rejected | retraction)",
        };
