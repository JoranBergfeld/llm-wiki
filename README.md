...
build, not the AOT one**, so it needs the .NET 9 runtime *or newer* present
(it rolls forward, so a machine with only 10.0 is fine), and GitHub
...
or any newer SDK that can still target `net9.0`).
